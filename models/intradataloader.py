...
class FAS_Dataset(Dataset):
    def __init__(self, root, protocol=['C','S','W','intra','inter'], train = True , size = 224):
        ...
        for i in protocol:
            if i == 'intra':
                ...
                # self.all_liver = np.load(root + '/train/real/rgb.npy')
                # self.all_lived = np.load(root + '/train/real/depth.npy')
                # self.all_livei = np.load(root + '/train/real/ir.npy')
                # self.all_spoofr= np.load(root + '/train/spoof/rgb.npy')
                # self.all_spoofd= np.load(root + '/train/spoof/depth.npy')
                # self.all_spoofi= np.load(root + '/train/spoof/ir.npy')
